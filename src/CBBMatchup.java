...
public class CBBMatchup
{
	...
	public static ArrayList<CBBMatchup> getCBBMatchups(LocalDate date)
	{
		...
		// combining all separate matchup objects from various sources into singular CBBMatchup objects
		// all valid matchups from Covers are used and then data from other sources is matched to Covers matchups
		...
		for (CoversCBBMatchup coversMatchup : coversCBBMatchups)
		{
			...
			for (int i = 0; i < oddsSharkCBBMatchups.size(); i++)
			{
				...
				if (temp.awayTeam.compareTo(oddsSharkAwayTeam) == 0)
				{
					...
					matchup.oddsSharkTotalScore = Double.parseDouble(numFormatter.format(temp.awayScore + temp.homeScore));
	...
	// reading in team names from csv file of all team names/abbreviations from various sources
